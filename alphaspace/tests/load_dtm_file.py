...
il2.apo1.80.33.mdm2.pdb  {system name}{prot}{.pdb}
il2.apo1.80.33.p53.pdb   {system name}{lig}{.pdb}
...
def run_alphaspace(files):
    ...
    universe = Trajectory(receptor=protein, binder=ligand)
    ...
    # universe = Trajectory()
    # for name in names:
    #     universe.set_receptor(structure=mdtraj.load(name + ".mdm2.pdb"), append=True, keepH=True)
    #     universe.set_binder(structure=mdtraj.load(name + ".p53.pdb"), append=True)
