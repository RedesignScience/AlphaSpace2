...
for file in glob.glob(pock_dir + "/[0-9]??_beta.pdb"):
	rc("open " + file )
pock_cnt = len(glob.glob(pock_dir + "/[0-9]??_beta.pdb"))
...
for i in range(pdb_cnt, models_cnt+1):
	...
	rc("color " + colors[i-3] +",a #" + str(i) + " & ~:PBC,BCC")
...
for i in range(pdb_cnt, models_cnt+1):
    rc("color " + colors[i-3] +",a #" + str(i) + ":BCC")
...
for i in range(pdb_cnt, models_cnt+1):
    rc("color " + colors[i-3] +",a #" + str(i) + ":BAC")
    rc("disp :BAC")
