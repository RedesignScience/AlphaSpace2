...
ADV_PARM = {"gauss_1": -0.035579,
            ...
            "Hydrogen": -0.587439}
ADV_score_terms = np.array([-0.035579, -0.005156, 0.840245, -0.035069, -0.587439])
# PROBE_TYPE = ['C', 'Br', 'F', 'Cl', 'I', 'OA', 'SA', 'N', 'P']
probe_element_list = ['C', 'Br', 'F', 'Cl', 'I', 'OA', 'SA', 'N', 'P']
"""
"""


def get_typing_dicts(hp_types_dat_path, typing_pdb_dat_path, autodock_atom_type_dat_path):
...
def pre_process_pdb(pdb_lines):
    ...
    Returns
    -------
    prot_coord, prot_types, hp_type, acc_type, don_type, autodock_types_dict
    """

    this_dir, this_filename = os.path.split(__file__)

    types_dict, typing_pdb_dict, cofactor_match_dict, autodock_types_dict = \
        get_typing_dicts(os.path.join(this_dir, "data", "hp_types_dict.dat"),
...
def pre_process_pdbqt(traj, truncation_length=0):
    """
        ...
        if atom.residue.name in types_dict.keys():
            if atom.name in types_dict[atom.residue.name].keys():
                if atom.pdbqt_name in ali_atoms:
                    ...
                    don_type.append('XXX')
                    acc_type.append('XXX')
                if atom.pdbqt_name in POLAR_ATOM_TYPE:
...
def gen_vina_type(atom_names, residue_names, elements,
    ...
    types_dict, typing_pdb_dict, cofactor_match_dict, autodock_types_dict = get_typing_dicts(
...
def get_probe_score(prot_coord, prot_types, hp_type, don_type, acc_type, probe_coords):
    ...
    probe_scores = np.zeros((probe_coords.shape[0], len(probe_element_list)), dtype=np.float32)
    ...
    for probe_idx in range(probe_coords.shape[0]):
        ...
        for element_idx, probe_element in enumerate(probe_element_list):
            probe_dist = ADV_DICT[probe_element][0]
