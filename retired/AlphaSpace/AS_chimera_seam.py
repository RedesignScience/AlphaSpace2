...
for file in glob.glob(pock_dir + "/[0-9]??.pdb"):
	rc("open " + file )
pock_cnt = len(glob.glob(pock_dir + "/[0-9]??.pdb"))
...
rc("color light grey,r #0:.A")
rc("color tan,r #0:.B")
rc("color byhet,a #0")

rc("surfcat surf_0 #0")
rc("surface surf_0")
rc("color white,s #0:.A")
rc("color tan,s #0:.B")
